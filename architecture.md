...
**Phase 7 implementation — four exception handlers registered in `core/errors.py`**, all 503 (upstream dependency failures, not client mistakes), all previously falling through to the generic `500 INTERNAL_ERROR`:

| Exception | error_code | Verified |
|---|---|---|
| `EmbeddingError` (`ingestion/embeddings.py`) | `OLLAMA_EMBEDDING_UNREACHABLE` | Exception type + message confirmed directly (Ollama stopped, `embed_text()` called in isolation); full HTTP round-trip via the required test case below still pending a working `ANTHROPIC_API_KEY` |
| `OllamaChatError` (`ollama_provider.py`) | `OLLAMA_CHAT_UNREACHABLE` | Full HTTP round-trip: `LLM_PROVIDER=ollama`, Ollama process killed, real `503` with attempted host in the message |
| SQLAlchemy `OperationalError` | `DATABASE_UNAVAILABLE` (generic message — not `str(exc)`, which can include connection details) | Full HTTP round-trip: `db` container stopped, `/health` returned `503`/`"degraded"`, a real request returned the structured `503`, both recovered cleanly after restart |
| `anthropic.APIError` (base class — confirmed via the actual class hierarchy: `AuthenticationError`, `BadRequestError`, `RateLimitError`, `APIConnectionError` all inherit from it, so one handler covers all of them) | `CLAUDE_PROVIDER_ERROR`, relaying the SDK's own message (already safe/informative) | Full HTTP round-trip, and not a synthetic test — this ran against a **real** live failure: an invalid key (401) and then a real insufficient-credit-balance error (400) both surfaced as clean structured `503`s instead of the generic `500` |

**`GET /health`** (previously liveness-only since Phase 1, despite this section's original text already describing the fuller behavior) now checks real DB connectivity (`SELECT 1`) and, for `LLM_PROVIDER=ollama`, a cheap `/api/version` ping; for `LLM_PROVIDER=claude` it reports `"configured"` rather than making a live billed call just to answer a health check. DB down returns `503`/`"degraded"` (nothing works without it); LLM provider down is reported as a field without failing the overall check (e.g. existing sessions stay readable even if new chat generation can't run).

**Required Phase 7 test case — status: designed and code-verified, full HTTP round-trip still pending.** `LLM_PROVIDER=claude`, a valid `ANTHROPIC_API_KEY` set, Ollama process stopped. The tool-routing decision itself requires a real, working Claude call before retrieval is ever reached, so this specific combination couldn't be exercised end-to-end while the Claude path was still blocked (first an invalid key, then insufficient credits — see agent-transcripts/README.md). The `EmbeddingError` handler and exception type are confirmed correct in isolation; closing the loop with the real HTTP flow is the first thing to re-run once Claude is actually reachable.
